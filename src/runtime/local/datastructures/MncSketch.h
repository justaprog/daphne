...
*It first checks if the simple diagonal condition is met if not, then we use
...
inline MncSketch propagateAdd(const MncSketch &A, const MncSketch &B) {
    MncSketch C;
    C.m = A.m;
    C.n = A.n;
...
inline MncSketch propagateMul(const MncSketch &A, const MncSketch &B) {
    MncSketch C;
    C.m = A.m;
    C.n = A.n;
    C.hr = std::make_shared<std::vector<uint32_t>>(C.m, 0);
